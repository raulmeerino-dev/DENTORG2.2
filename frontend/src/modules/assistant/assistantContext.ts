...
export function buildAssistantContext(pathname: string, user: UsuarioMe | null): AssistantContextSnapshot {
  const patientId = user?.rol === 'paciente'
    ...
    : readSessionValue('dentorg_selected_patient_id');
  const patientName = user?.rol === 'paciente'
    ? null
    : readSessionValue('dentorg_selected_patient_name');
  ...
  return {
    ...
    selectedAppointmentId: readSessionValue('dentorg_agenda_focus_cita_id'),
    visibleAgendaDate: readSessionValue('dentorg_agenda_focus_date'),
