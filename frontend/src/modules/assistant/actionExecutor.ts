...
function setPatientSession(intent: AssistantIntent) {
  ...
  sessionStorage.setItem('dentorg_selected_patient_id', intent.fields.patientId);
  if (intent.fields.patientDisplayName) {
    sessionStorage.setItem('dentorg_selected_patient_name', intent.fields.patientDisplayName);
  }
}
...
export async function executeAssistantAction({
  ...
  switch (intent.intent) {
    ...
    case 'create_appointment': {
      ...
      if (!professionalId) {
        return { ok: false, message: 'No puedo crear la cita sin un profesional real de DentOrg.' };
      }
      if (!slot?.fechaHora) {
