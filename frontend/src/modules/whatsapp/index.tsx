...
export default function WhatsAppPage() {
  ...
  function openPatient(item: WhatsAppInboxItem) {
    ...
    sessionStorage.setItem('dentorg_selected_patient_id', item.patient_id);
    navigate(`/pacientes?paciente_id=${item.patient_id}`);
  }

  function openAgenda(item: WhatsAppInboxItem) {
    if (item.appointment) {
      sessionStorage.setItem('dentorg_agenda_focus_cita_id', item.appointment.id);
      sessionStorage.setItem('dentorg_agenda_focus_date', item.appointment.fecha_hora.slice(0, 10));
    }
    navigate('/agenda');
  }
