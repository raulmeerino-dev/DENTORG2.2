...
export default function AgendaPage() {
  ...
  const [doctorId, setDoctorId] = useState<string>(() => searchParams.get('doctor_id') ?? '');
  ...
  }, [day, doctorId, searchParams]);

  useEffect(() => {
    const focusCitaId = searchParams.get('cita_id') ?? sessionStorage.getItem('dentorg_agenda_focus_cita_id');
    ...
    return () => window.clearTimeout(timeout);
  }, [citas, citasQuery.isLoading, searchParams]);
  ...
  function buscarCita() {
    const selectedPacienteId = sessionStorage.getItem('dentorg_selected_patient_id');
  ...
  return (
    <section className="page page-shell agenda-euro" onClick={() => setContextMenu(null)}>
      ...
      {showCitasPaciente && sessionStorage.getItem('dentorg_selected_patient_id') && (
        <CitasPacienteModal
          pacienteId={sessionStorage.getItem('dentorg_selected_patient_id')!}
          pacientes={pacientes}
