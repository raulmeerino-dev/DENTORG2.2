              ...
              <tbody>
                {(telefonearQuery.data ?? []).map((item: TelefonearPendiente) => (
                  <tr
                    ...
                    draggable
                    onDragStart={(event) => {
                      event.dataTransfer.setData('application/dentcore-patient', item.paciente ? JSON.stringify({ pacienteId: pacientes.find((p) => p.telefono === item.paciente?.telefono)?.id, name: `${item.paciente.apellidos}, ${item.paciente.nombre}` }) : '');
                    }}
                  >
