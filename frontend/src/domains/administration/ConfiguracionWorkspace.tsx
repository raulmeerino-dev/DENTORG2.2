...
import { ROLE_LABELS, WORKFLOW_ITEMS, canRoleAccess } from '../../app/navigation/workflow';
import { crearBackup, descargarBackup, getBackups, getProductionReadiness, registrarPruebaRestauracionBackup, simularRestauracionBackup, verificarBackup } from '../../api/administration';
import { createDoctor, getDoctores, updateDoctor } from '../../api/identity';
import { createFamiliaTratamiento, createTratamientoCatalogo, deactivateTratamientoCatalogo, getFamiliasTratamiento, getTratamientosCatalogo, updateTratamientoCatalogo } from '../../api/treatmentCatalog';
import { getFormasPago } from '../../api/billing';
import { getHorarios, updateHorarioDoctor } from '../../api/scheduling';
import { getLaboratorios } from '../../api/laboratory';
...
function formFromTreatment(tratamiento: TratamientoCatalogo): TreatmentForm {
  return {
    ...
    duracion_habitual_min: tratamiento.duracion_habitual_min == null ? '' : String(tratamiento.duracion_habitual_min),
...
export function ConfiguracionWorkspace({
  ...
  const saveHorarioMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      window.dispatchEvent(new CustomEvent('dentcore:horarios-updated'));
      window.localStorage.setItem('dentcore_horarios_updated_at', String(Date.now()));
  ...
  const saveTreatmentMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        duracion_habitual_min: tratamientoForm.duracion_habitual_min ? Number(tratamientoForm.duracion_habitual_min) : null,
      ...
      if (payload.duracion_habitual_min !== null && (!Number.isInteger(payload.duracion_habitual_min) || payload.duracion_habitual_min < 5 || payload.duracion_habitual_min > 480 || payload.duracion_habitual_min % 5)) throw new Error('La duración habitual debe estar entre 5 y 480 minutos, en intervalos de 5.');
