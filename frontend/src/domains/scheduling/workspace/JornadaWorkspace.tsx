...
function JornadaContent() {
  ...
  const perspective = params.get('vista') === 'agenda' ? 'agenda' : 'operativa';
  function changePerspective(vista: string) {
    setParams(current => { const next = new URLSearchParams(current); next.set('vista', vista); return next; });
  }
  return <section className="jornada-workspace" aria-label="Jornada">
    ...
    <div className="jornada-filters" aria-label="Filtros de Jornada">
