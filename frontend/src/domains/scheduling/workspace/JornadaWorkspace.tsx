...
function JornadaContent() {
  ...
  return <section className={`jornada-workspace${perspective === 'agenda' ? ' agenda-workspace-active' : ''}`} aria-label="Jornada">
    ...
    <ContextToolbar aria-label="Acciones de Jornada" className={perspective === 'agenda' ? 'agenda-controls-toolbar' : undefined}>
      {perspective === 'agenda' && <>
        <ToolbarSearch aria-label="Buscar en Jornada" placeholder="Buscar paciente o cita…" value={jornada.searchQuery} onChange={e => jornada.setSearchQuery(e.target.value)} />
        <AgendaPeriodSelectors compact day={jornada.day} onChange={jornada.setDay} />
        <select className="agenda-professional-select" aria-label="Profesional de Agenda" value={jornada.doctorId} onChange={e => jornada.setDoctorId(e.target.value)}>
