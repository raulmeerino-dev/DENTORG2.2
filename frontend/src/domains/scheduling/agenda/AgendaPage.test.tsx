...
vi.mock('../../../api/scheduling', () => ({
  ...
  getGabinetes: vi.fn().mockResolvedValue([]),
  getJornadaConfig: vi.fn().mockResolvedValue({ espera_aviso_min: 10, espera_critica_min: 25, duracion_habitual_min: 30 }),
  marcarLlegadaCita: vi.fn(async () => ({ ...mocks.cita, estado: 'en_clinica', estado_operativo: 'en_sala' })),
  iniciarAtencionCita: vi.fn(async () => ({ ...mocks.cita, estado: 'en_atencion', estado_operativo: 'en_atencion' })),
  finalizarVisitaCita: vi.fn(async () => ({ ...mocks.cita, estado: 'atendida', estado_operativo: 'finalizada' })),
  createPacienteProvisional: vi.fn(),
  marcarFaltaCita: vi.fn(),
  updateCita: mocks.updateCita,
}));

vi.mock('../../../api/treatmentCatalog', () => ({ getTratamientosCatalogo: vi.fn().mockResolvedValue([]) }));
...
describe('AgendaPage flujos de cita', () => {
  ...
  it('no duplica el filtro activo ni estados equivalentes en agenda', async () => {
    ...
    expect(within(legend).queryByText('Mensaje enviado')).not.toBeInTheDocument();
    expect(within(legend).getAllByText('En atención')).toHaveLength(1);
  });
  ...
  it('abre nueva cita desde Pacientes aunque el dia no tenga horario configurado', async () => {
    ...
    expect(await screen.findByText(/^Nueva cita$/i)).toBeInTheDocument();
    expect(screen.getByText(/Historia 91312/)).toBeInTheDocument();
    expect(screen.getByLabelText('Hora inicio')).toHaveValue('09:00');
  });

  it('crea una cita confirmada de forma atómica sin una segunda mutación', async () => {
    const user = userEvent.setup();
    sessionStorage.setItem('dentcore_agenda_action', 'new');
    sessionStorage.setItem('dentcore_selected_patient_id', mocks.paciente.id);
    renderAgenda();
    const confirmed = await screen.findByLabelText('Cita confirmada por el paciente');
    await user.click(confirmed);
    expect(mocks.createCita).not.toHaveBeenCalled();
    await user.click(screen.getByRole('button', { name: /Guardar cita/i }));
    await waitFor(() => expect(mocks.createCita).toHaveBeenCalledWith(expect.objectContaining({ estado: 'confirmada' })));
    expect(mocks.updateCita).not.toHaveBeenCalled();
  });

  it('reserva las transiciones clínicas para las acciones auditadas de una cita existente', async () => {
    sessionStorage.setItem('dentcore_agenda_action', 'new');
    sessionStorage.setItem('dentcore_selected_patient_id', mocks.paciente.id);
    renderAgenda();
    await screen.findByLabelText('Cita confirmada por el paciente');
    expect(screen.queryByRole('group', { name: /Estado de la cita/i })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /En tratamiento/i })).not.toBeInTheDocument();
  });
  ...
  it('al arrastrar Telefonear a un hueco crea cita real y marca la entrada como reubicada', async () => {
    ...
    const slot = await waitFor(() => {
      const target = container.querySelector('.agenda-resource-cell');
