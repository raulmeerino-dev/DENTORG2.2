...
export function CancelCitaModal({
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <section {...dialog} aria-label={estado === 'falta' ? 'Registrar falta' : 'Cancelar cita'} className="patient-edit-modal" style={{ maxWidth: 480 }} onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="patient-edit-grid" style={{ padding: '1rem', gap: '0.75rem', display: 'flex', flexDirection: 'column' }}>
          <p style={{ margin: 0 }}>
            <strong>{pacienteNombre}</strong> — {formatDate(cita.fecha_hora)} {localAppointmentTime(cita.fecha_hora)}
