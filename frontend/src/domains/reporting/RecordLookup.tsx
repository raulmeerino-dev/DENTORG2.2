...
export function RecordLookup({ label, kind, value, onChange, scope }: {
  ...
  function toggle() {
    if (!open && trigger.current) {
      ...
      const height = Math.min(340, Math.max(120, downward ? below : above));
      setPlacement({ position: 'fixed', width, maxHeight: height, left: Math.max(12, Math.min(anchor.left, window.innerWidth - width - 12)), ...(downward ? { top: anchor.bottom + 4 } : { top: 'auto', bottom: window.innerHeight - anchor.top + 4 }) });
    }
    setOpen(!open);
  }
  ...
  return <div className="records-lookup" ref={root} onKeyDown={event => {
    ...
    {open && <div className="records-lookup-popover" style={placement}>
