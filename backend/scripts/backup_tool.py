"""Herramienta offline para verificar y extraer backups DentOrg2.

Uso previsto:
  python -m scripts.backup_tool verify --file backup.dentorg2bak --expected-hash ...
  python -m scripts.backup_tool extract --file backup.dentorg2bak --output-dir restore-kit
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verifica o extrae backups cifrados de DentOrg2.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    verify = subparsers.add_parser("verify", help="Descifra y valida un backup sin extraerlo.")
    verify.add_argument("--file", required=True, type=Path, help="Ruta del fichero .dentorg2bak.")
    ...
    extract.add_argument("--file", required=True, type=Path, help="Ruta del fichero .dentorg2bak.")
