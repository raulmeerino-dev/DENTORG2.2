...
from app.domains.clinical.application.receta_provider_service import (
    get_receta_provider,
...
async def _get_doctor(db: AsyncSession, doctor_id: uuid.UUID, paciente: Paciente, current_user: TokenData) -> Doctor:
...
async def _get_plantilla(db: AsyncSession, plantilla_id: uuid.UUID, current_user: TokenData) -> RecetaPlantilla:
...
async def receta_provider_status(_: TokenData) -> RecetaProviderStatus:
    return RecetaProviderStatus(**provider_status_payload())


async def listar_plantillas(db: AsyncSession, current_user: TokenData) -> list[RecetaPlantillaResponse]:
...
async def importar_plantilla(request: Request, db: AsyncSession, current_user: TokenData, archivo: UploadFile, nombre: str, campos_config: str | None, requiere_dni: bool, requiere_fecha_nacimiento: bool) -> RecetaPlantillaResponse:
...
async def actualizar_plantilla(plantilla_id: uuid.UUID, data: RecetaPlantillaUpdate, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaPlantillaResponse:
...
async def crear_receta(paciente_id: uuid.UUID, data: RecetaCreate, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    paciente = await _get_paciente(db, paciente_id, current_user)
...
async def obtener_receta(receta_id: uuid.UUID, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
    return _to_response(receta)


async def actualizar_receta(receta_id: uuid.UUID, data: RecetaUpdate, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
    if receta.estado in FINAL_STATES:
...
async def firmar_receta(receta_id: uuid.UUID, data: RecetaFirmaUpdate, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
    if receta.estado in FINAL_STATES:
...
async def emitir_receta_local(receta_id: uuid.UUID, data: RecetaEmitirRequest, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
    paciente = await _get_paciente(db, receta.paciente_id, current_user)
...
async def enviar_receta_proveedor(receta_id: uuid.UUID, data: RecetaEmitirRequest, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
...
async def anular_receta(receta_id: uuid.UUID, data: RecetaAnularRequest, request: Request, db: AsyncSession, current_user: TokenData) -> RecetaResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
...
async def pdf_receta(receta_id: uuid.UUID, request: Request, db: AsyncSession, current_user: TokenData) -> FileResponse:
    receta = await _get_receta_or_404(db, receta_id, current_user)
