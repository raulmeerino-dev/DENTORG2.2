...
from app.domains.clinical.domain.dental_surfaces import surfaces_from_caras
...
def _ensure_clinical_access(user: TokenData) -> None:
...
def _ensure_modify_access(user: TokenData) -> None:
...
async def _get_patient(db: AsyncSession, paciente_id: UUID, user: TokenData) -> Paciente:
...
async def _load_odontograma(db: AsyncSession, odontograma_id: UUID, user: TokenData) -> Odontograma:
...
def _apply_context_tooth(
    ...
    *,
    ...
    for surface in surfaces_from_caras(caras):
...
async def obtener_odontograma_paciente(paciente_id: UUID, request: Request, current_user: TokenData, db: AsyncSession) -> OdontogramaResponse:
...
async def obtener_odontograma_contexto(paciente_id: UUID, request: Request, current_user: TokenData, mode: OdontogramaContextMode, context_id: UUID | None, db: AsyncSession) -> OdontogramaContextResponse:
    _ensure_clinical_access(current_user)
    paciente = await _get_patient(db, paciente_id, current_user)
    odontograma = await _active_odontograma(db, paciente.id)
    if not odontograma:
...
async def crear_odontograma_paciente(paciente_id: UUID, request: Request, current_user: TokenData, db: AsyncSession) -> OdontogramaResponse:
    _ensure_clinical_access(current_user)
...
async def actualizar_pieza(odontograma_id: UUID, pieza_fdi: int, data: OdontogramaPiezaUpdate, request: Request, current_user: TokenData, db: AsyncSession) -> OdontogramaPiezaResponse:
    _ensure_clinical_access(current_user)
    _ensure_modify_access(current_user)
...
async def actualizar_superficie(odontograma_id: UUID, pieza_fdi: int, superficie: SuperficieDental, data: OdontogramaSuperficieUpdate, request: Request, current_user: TokenData, db: AsyncSession) -> OdontogramaSuperficieResponse:
    _ensure_clinical_access(current_user)
    _ensure_modify_access(current_user)
    _validate_adult_tooth(pieza_fdi)
...
async def crear_presupuesto_desde_plan(odontograma_id: UUID, data: PlanTratamientoCreate, request: Request, current_user: TokenData, db: AsyncSession) -> PlanTratamientoResponse:
    _ensure_clinical_access(current_user)
    _ensure_modify_access(current_user)
    odontograma = await _load_odontograma(db, odontograma_id, current_user)
...
async def historial_odontograma(odontograma_id: UUID, current_user: TokenData, db: AsyncSession) -> list[OdontogramaEventoResponse]:
    _ensure_clinical_access(current_user)
...
async def duplicar_version_odontograma(odontograma_id: UUID, request: Request, current_user: TokenData, db: AsyncSession) -> OdontogramaResponse:
    _ensure_clinical_access(current_user)
    _ensure_modify_access(current_user)
