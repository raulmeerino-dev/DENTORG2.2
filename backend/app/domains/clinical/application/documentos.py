...
async def listar_documentos(paciente_id: uuid.UUID, db: AsyncSession, current_user: TokenData, categoria: str | None):
...
async def subir_documento(paciente_id: uuid.UUID, request: Request, db: AsyncSession, current_user: TokenData, archivo: UploadFile, categoria: str, descripcion: str | None, fecha_documento: date | None, tratamiento_id: uuid.UUID | None, historial_id: uuid.UUID | None, doctor_id: uuid.UUID | None, etiquetas: str | None):
    pac = await db.get(Paciente, paciente_id)
    if not pac:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
    ensure_clinic_access(current_user, pac.clinica_id)
...
async def generar_documento_pdf(paciente_id: uuid.UUID, data: DocumentoPdfCreate, db: AsyncSession, current_user: TokenData, request: Request):
    pac = await db.get(Paciente, paciente_id)
    if not pac:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
    ensure_clinic_access(current_user, pac.clinica_id)
...
async def descargar_documento(paciente_id: uuid.UUID, doc_id: uuid.UUID, db: AsyncSession, current_user: TokenData):
...
async def eliminar_documento(paciente_id: uuid.UUID, doc_id: uuid.UUID, db: AsyncSession, current_user: TokenData, request: Request, motivo: str | None):
    doc = await db.get(DocumentoPaciente, doc_id)
    if not doc or doc.paciente_id != paciente_id or doc.deleted_at is not None:
        raise HTTPException(status_code=404, detail="Documento no encontrado")
    pac = await db.get(Paciente, paciente_id)
